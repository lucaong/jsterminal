...
JSterminal.meta.printWaitingOutput = function() {
  var io = JSterminal.ioQueue.firstActive();
  if(!!io) {
    for (out in io.outputQueue) {
      jQuery("#JSterminal_in_wrap").before((this.meta.prefixes.output || "")+(io.outputQueue[out]||"")+"\n");
      jQuery("#JSterminal_out").scrollTop(jQuery("#JSterminal_out").attr("scrollHeight"));
      jQuery("#JSterminal_in").focus();
    }
  }
...
JSterminal.eventHandlers.ioReleased = function() {
  JSterminal.meta.printWaitingOutput();
}
